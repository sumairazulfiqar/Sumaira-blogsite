...
if(isset($_POST['edit-profile'])){
    ...
    $admin_linkdin = mysqli_real_escape_string($cn, $_POST['linkedin']);

        //cod for image update
        if(empty($_FILES['new_admin_img']['name'])){
            $admin_img = $_POST['old_admin_img'];
        }
           else{
    ...
    $sql = "UPDATE `admin` SET `admin_name`='$admin_name',`admin_mobile`='$admin_mobile',`admin_country`='$admin_country',`admin_profession`='$admin_profession',`admin_image`='$admin_img',`admin_about`='$about_admin',`admin_facebook`='$admin_facebook',`admin_twitter`='$admin_twitter',`admin_instagram`='$admin_instagram',`admin_linkdln`='$admin_linkdin' WHERE admin_id = '$admin_id'" ;
